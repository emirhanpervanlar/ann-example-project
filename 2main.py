...
def nn_test():
    ...
    rand_val1 = random.randint(0, len(input_data)-1)
    rand_val2 = random.randint(0, len(input_data)-1)

    rand_i_data = list()
    rand_c_data = list()
    rand_i_data.append(input_data[rand_val1])
    rand_i_data.append(input_data[rand_val2])
    rand_c_data.append(class_data[rand_val2])
    rand_c_data.append(class_data[rand_val2])

    print("Test için rasgele 1.örnek indisi "+str(rand_val1)+"  Test için rasgele 2.örnek indisi "+str(rand_val2))
    print("İnput matrisi : ")
    print(rand_i_data)
    print("Output matrisi : ")
    print(rand_c_data)
    input_data = rand_i_data
    target_data = rand_c_data
    if epoch <= 0:
        messagebox.showinfo("Hata", "Epoch değeri hatalı girildi")
    elif e_tolerance < 0:
        messagebox.showinfo("Hata", "Hata toleransı değeri hatalı girildi.0-1 arası değer giriniz.")
...
if __name__ == "__main__":
    ...
    lb5 = Label(master, text="Toplam Örnek Sayısı : 9")
    lb6 = Label(master, text="Eğitim Örnek Sayısı : 5")
    lb0 = Label(master, text="Test Ağı Örnek Sayısı : 4")
    ...
    education_btn = Button(text="Ağı Eğit", command=nn_pred,padx=20,pady=5)

    test_btn = Button(text="Ağı Test Et", command=nn_test,padx=20,pady=5)

    draw_btn = Button(text="Ağı Çiz", command=draw_neural_net,padx=20,pady=5)
